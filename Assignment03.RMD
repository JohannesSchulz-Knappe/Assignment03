---
title: "Assignment 3 - Data Set Generation and First Analysis"
...
1) The first data type is the electoral data provided by the State Offices for Statistics on the last state elections in  Baden-Württemberg [@BWStateOffice], Rhineland-Palatinate [@RPStateOffice], and Saxony-Anhalt [@SAStateOffice]. The variables retrieved from these datasets are:

 - Dependent Variable (DV): vote.AfD
 The vote share of the Alternative for Germany in the current state election
   
 - Independent Variable 1 (IV1): lag.turnout
 Overall voter turnout of the previous state election
   
 - IV2, IV3, IV4, IV5, IV6: lag.CDU, lag.Greens, lag.SPD, lag.FDP, lag.Linke
 The vote share of political parties in the previous state election (CDU, SPD, Greens, FDP, and Linke)
 

2) The second data type is on the structural characteristics on the administrative districts. We retrieve this data from the Federal Statistical Office DESTATIS [@DESTATIS]. As this data is only available on district level, we adapted our data collection on it. The variables retrieved from these datasets are:

 - IV7: abitur.ratio 
 Ratio of school leavers per district with general qualification for university entrance (Abitur) 
  
 - IV8: nodegree.ratio
 Ratio of school leavers per district with no school degree
 ...
 - IV11: n.refugees
 Number of asylum seekers per district
...
# Empirical Findings
With the generated dataset we were able to conduct initial analysis of correclations and distributions. While our dependent varialbe is a [0, 1] bounded variable (meaning, the vote share can take values between 0% and 100%), we will use beta regression for our final analysis. For the beginning, we use OLS to get a basic idea of the relationships of the selected variables.

## Descriptive Statistics
Table 1 shows general descriptive statistics of our variables. Except for the "GDP per capita" and "number of refugees" variables they are all in percent. 
...
summary_labels <- c('Vote share of AfD in 2016', 'Vote turnout in 2011', 
                    ...
                    'No school degree ratio', 'GDP per capita', 
...
Table 2 provides a correlation matrix of a selection of our variables. 
...
The following graph shows a slightly statistically significant (t value is -2.04) negative coorelation of GDP per capita with AfD vote share. The effect seems to be mostly influenced by a small group of outliers (high vote share, low GDP per capita) and the variance is not equal across the indipendent variable (posing a problem with heteroscedasticity).

``` {r fig1, echo = F, message = F, results = "hide", error=F, fig.height=4}
ggplot2::ggplot(Data,aes(log(x = GDP.capita), vote.AfD)) + 
  xlab("GDP per capita per district (in ???)") +
